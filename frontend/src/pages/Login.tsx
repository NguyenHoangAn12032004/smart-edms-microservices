...
export function Login() {
  ...
  // States for First-Time Login Password Change Modal
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.token) {
        // UI Logic Simulation: If backend flags this as a newly created account (first login)
        if (data.isFirstLogin) {
          // Temporarily hold token to use for changing password
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // UI Only: Dispatch request to backend
      const resp = await fetch("http://localhost:8080/api/auth/change-password", {
        method: "POST",
        headers: {
      ...
      // Success: Apply token fully and login
      ...
      setChangePwdError(err.message || "An error occurred");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden relative">
      ...
      {/* First Time Login: Change Password Modal */}
      <AnimatePresence>
        {showChangePasswordModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-surface border border-border rounded-2xl shadow-2xl p-6 lg:p-8 overflow-hidden"
            >
              {/* Glow effect */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-32 bg-primary/20 blur-3xl opacity-50 pointer-events-none" />
              ...
              <div className="relative">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-foreground">Secure your account</h3>
                    <p className="text-sm text-muted-foreground">Please change your default password</p>
                  </div>
                </div>
                ...
                <form onSubmit={handleChangePassword} className="space-y-5">
                  <div>
                    ...
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        required
                        minLength={6}
                        className="w-full bg-background border border-border rounded-xl px-12 py-3 text-foreground placeholder-muted-foreground focus:outline-none focus:border-primary transition-colors font-medium"
                  ...
                  <button
                    ...
                    className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl py-3.5 flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                  >
                    {isChangingPwd ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
